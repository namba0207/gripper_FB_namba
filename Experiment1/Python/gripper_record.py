...
class Text_class:
    def __init__(self):
        ...
        thr1 = threading.Thread(target=self.loadcell_thread)
        thr1.setDaemon(True)
        thr1.start()
        thr2 = threading.Thread(target=self.motor_thread)
        thr2.setDaemon(True)
        thr2.start()
        thr3 = threading.Thread(target=self.recode_thread)
        thr3.setDaemon(True)
        thr3.start()
    ...
    def recode_thread(self):
        while True:
            self.gripper = self.arm.get_gripper_position()[1]
            time.sleep(0.005)
...
if __name__ == "__main__":
    ...
    while True:
        try:
            data1 = text_class.gripper
            data2 = text_class.num_int
            data3 = time.perf_counter() - start_time
            # 新しいデータをCSVファイルに追記
            with open("data0115_3.csv", "a", newline="") as file:
