...
class ParticipantMotionManager:
    ...
    def LocalPosition(self, loopCount: int = 0):
        ...
        if self.motionInputSystem == "optitrack":
            ...
            position = np.array(
                [
                    float(data[data.index("pos") + 1]),
                    float(data[data.index("pos") + 2]),
                    float(data[data.index("pos") + 3]),
                ]
            )
        ...
        for i in range(self.recordedParticipantNum):
            ...
            if loopCount >= self.recordedMotionLength[i]:
                dictPos["participant" + str(recordedParticipantNum)] = np.array(
                    self.recordedMotion["participant" + str(recordedParticipantNum)][
                        -1
                    ][0:3]
                )
                continue
    ...
    def LocalRotation(self, loopCount: int = 0):
        ...
        if self.motionInputSystem == "optitrack":
            ...
            if "rotEuler" in data:
                rotation = (
                    float(data[data.index("rotEuler") + 1]),
                    float(data[data.index("rotEuler") + 2]),
                    float(data[data.index("rotEuler") + 3]),
                )
            elif "rotQuaternion" in data:
                rotation = np.array(
                    [
                        float(data[data.index("rotQuaternion") + 1]),
                        float(data[data.index("rotQuaternion") + 2]),
                        float(data[data.index("rotQuaternion") + 3]),
                        float(data[data.index("rotQuaternion") + 4]),
                    ]
                )
        ...
        for i in range(self.recordedParticipantNum):
            recordedParticipantNum = self.defaultParticipantNum + i + 1
            if loopCount >= self.recordedMotionLength[i]:
                dictRot["participant" + str(recordedParticipantNum)] = np.array(
                    self.recordedMotion["participant" + str(recordedParticipantNum)][
                        -1
                    ][3:8]
                )
    ...
    def GripperControlValue(self, loopCount: int = 0):
        ...
        if self.gripperInputSystem == "bendingsensor":
            ...
            if "trigger" in data:
                ...
                triggerValueNorm = (triggerValue - originalMin) / (
        ...
        for i in range(self.recordedGripperValueNum):
            ...
            if loopCount >= self.recordedMotionLength[i]:
                dictGripperValue["gripperValue" + str(recordedGripperNum)] = np.array(
                    self.recordedGripperValue["gripperValue" + str(recordedGripperNum)][
                        -1
                    ][0]
                )
                continue
