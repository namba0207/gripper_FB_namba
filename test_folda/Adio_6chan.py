...
def main():
    ...
    data_rec = []
    # data_sps = 10000
    # 前回のデータを保存する辞書（最初は0で初期化）
    prev_data = {f"y{i+1}": 0 for i in range(REQUEST_DATA_NUM)}
    start_time = time.perf_counter()
    while True:
        try:
            x += 1
            send_data = {"x": x}
            # send_data["y0"] = time.perf_counter() - start_time
            ...
            for _ in range(REQUEST_DATA_NUM):
                ...
                if parsed is not None:
                    ch, data = parsed
                    prev_data[f"y{ch+1}"] = data  # 前回のデータを更新

            # 常に `y1` 〜 `y6` を送る（データがない場合は前回の値を保持）
            send_data.update(prev_data)
            ...
            # データをappend
            data_rec.append(send_data)

        except KeyboardInterrupt:
            finish_time = time.perf_counter() - start_time
            print(finish_time)
            with open('data0211_.txt','w') as f:
                f.write(str(finish_time)+'sps')
                f.write('\n')
                for d in data_rec:
                    f.write(str(d))
                    f.write('\n')
        ...
        #     with open('data0207_.txt', 'w') as f:
        #         f.write('Time,y1,y2,y3,y4,y5,y6' + '\n')
        #         for d in data_rec:
        #             for c in range(CHUNK_SIZE):
        #                 data_log = []
        #                 data_log.append(((d['x']-1)*CHUNK_SIZE + c + 1)/data_sps)
        #                 # print(d['x'])
        #                 for key,item in d.items():  # 辞書の値を取得
        #                     if key == 'x':
        #                         continue
        #                     else:
        #                         data_log.append(item[c])
        #                 f.write(', '.join(map(str, data_log)) + '\n')
        #     break
